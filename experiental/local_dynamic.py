...
MAX_THREADS = 10  # Define an upper limit for safety
...
class MonitoringThread(threading.Thread):
    def __init__(
        ...
        super().__init__()
        self.desired_request_rate = desired_request_rate
        self.stats_tracker = stats_tracker
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            error = self.desired_request_rate - actual_rate
            # Use proportional control to adjust thread count
            Kp = 0.5  # Proportional gain, adjust as needed
            delta_threads = int(Kp * error)
            with self.scraper_threads_lock:
                current_thread_count = len(self.scraper_threads)
                # Adjust delta_threads to not exceed MAX_THREADS
                if delta_threads > 0:
                    threads_to_add = min(
                        delta_threads, MAX_THREADS - current_thread_count
                    )
                    threads_to_add = max(threads_to_add, 0)
                    ...
                    threads_to_remove = min(
                        -delta_threads, current_thread_count
                    )
