...
offset = 0  # Global variable to manage pause state
...
class ScraperThread(threading.Thread):
    ...
    def run(self):
        global offset  # ADDED: Declare offset as global
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if "rate_limit_reached" in error.lower():  # CHANGED: Detect rate limit based on error
                    ...
                    offset = RATE_LIMIT_WAIT  # ADDED: Set offset to initiate pause
...
class URLFeederThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.index < len(self.urls) and not self.stop_event.is_set():
            if offset:  # ADDED: Check for pause
                time.sleep(offset)
                offset = 0  # Reset offset after waiting
